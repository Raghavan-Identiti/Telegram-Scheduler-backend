...
#     """
#     Telegram's max message size is 4096 characters.
#     This function splits longer messages into parts under that limit.
#     """
...
#             # Send image with first part as caption (caption limit ~1024)
#             caption = message_parts[0][:1024]  # Ensure caption is within limit
#             await client.send_file('me', image_path, caption=caption)
#             for part in message_parts[1:]:
#                 await client.send_message('me', part)
#             status = 'Image + Text sent (split)'
...
#                 await client.send_message('me', part)
#             status = 'Text only sent (split if needed)'
...
async def send_telegram_message(image_path: str, post_text: str, post_number: int = 1, category: str = None, target: str = 'me',schedule_time: datetime = None):
    ...
    try:
        if image_path and message_parts:
            ...
            await client.send_file(target, image_path, caption=caption, schedule=schedule_time)
            for part in message_parts[1:]:
                await client.send_message(target, part, schedule=schedule_time)
            status = f'Scheduled Image + Text at {schedule_time}'

        elif image_path and not message_parts:
            await client.send_file(target, image_path, schedule=schedule_time)
